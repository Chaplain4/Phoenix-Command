...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QTabWidget,
                              ...
                              QLabel, QSplitter, QLineEdit, QCheckBox)
...
class EquipmentDialog(QDialog):
    ...
    def __init__(self, character: Character, parent=None):
        ...
        self._ammo_by_name: dict[str, AmmoType] = {}
        self._setup_ui()
        self._load_items()
    ...
    def _setup_ui(self):
        ...
        self.ammo_weapon_filter = QCheckBox("Only ammo for equipped weapons")
        self.ammo_weapon_filter.setVisible(False)
        self.ammo_weapon_filter.toggled.connect(self._filter_items)
        left_layout.addWidget(self.ammo_weapon_filter)
    ...
    def _on_tab_changed(self, index: int):
        """Show ammo filter checkbox only on the Ammo tab."""
        self.ammo_weapon_filter.setVisible(index == self.AMMO_TAB_INDEX)
        self._filter_items()
            ...
            for name, obj in weapons.__dict__.items():
                if isinstance(obj, Weapon):
                    ...
                    for ammo in obj.ammunition_types:
                        if ammo.name not in self._ammo_by_name:
                            self._ammo_by_name[ammo.name] = ammo
    ...
    def _populate_lists(self):
        ...
        for armor_item in self.all_armor:
            self.armor_list.addItem(armor_item.name)
    ...
    def _get_compatible_ammo_names(self) -> set[str]:
        """Ammo names compatible with weapons currently on the character."""
        names: set[str] = set()
        for item in self.character.equipment:
            if isinstance(item, Weapon):
                for ammo in item.ammunition_types:
                    names.add(ammo.name)
        return names

    def _filter_items(self):
        """Filter items based on search and ammo weapon filter."""
        search = self.search_input.text().lower()
        compatible_ammo = self._get_compatible_ammo_names()
        weapon_filter_active = (
            self.tabs.currentIndex() == self.AMMO_TAB_INDEX
            and self.ammo_weapon_filter.isChecked()
        )
        ...
        for i, lst in enumerate(lists):
            for j in range(lst.count()):
                item = lst.item(j)
                matches_search = search in item.text().lower()
                if i == self.AMMO_TAB_INDEX and weapon_filter_active:
                    matches_weapon = item.text() in compatible_ammo
                    item.setHidden(not (matches_search and matches_weapon))
                else:
    ...
    def _show_item_details(self, current, previous):
        ...
        if current_tab == 0:
            ...
            item = self._ammo_by_name.get(item_name)
        elif current_tab == 3:
